...
class Escudo(Collectible):
    """Clase de escudo que hereda de Collectible - proporciona proteccion temporal"""
    ...
    def __init__(self, resource_manager, *groups, width: int = DEFAULT_PILAS_WIDTH, height: int = DEFAULT_PILAS_HEIGHT):
        ...
        # Configuracion especifica del escudo
        self.protection_duration = ESCUDO_DURACION  # Duracion de la proteccion en segundos
        self.effect_duration = self.protection_duration
    
    def _apply_effect(self, robot):
        """Aplica el efecto especifico del escudo - otorga proteccion temporal"""
        if hasattr(robot, 'shield_time'):
            robot.shield_time = self.protection_duration
            robot.has_shield = True
            print(f"¡Escudo activado! Proteccion por {self.protection_duration} segundos")
        else:
            print("Robot no compatible con sistema de escudo")
    
    def _play_collection_sound(self):
        """Reproduce el sonido de recoleccion de escudo"""
        # Reproducir sonido especifico para escudos
        if hasattr(self.resource_manager, 'play_sound'):
            self.resource_manager.play_sound("collect_shield")
    
    def get_effect_info(self) -> str:
        """Retorna informacion sobre el efecto del escudo"""
        return f"Proteccion durante {self.protection_duration} segundos contra colisiones"
    ...
    def create_collision_visual_effect(screen, player_rect, camera_x):
        """Crea efecto visual cuando el escudo absorbe una colision"""
        # Posicion en pantalla
        screen_x = player_rect.x - camera_x + player_rect.width // 2
        screen_y = player_rect.y + player_rect.height // 2
        
        # Efecto de explosion/impacto
        current_time = pygame.time.get_ticks()
        
        # Multiples anillos expansivos
        for ring in range(3):
