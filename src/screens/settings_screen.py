...
class SettingsScreen(Scene):
    ...
    def _create_buttons(self):
        """Crea los botones de configuración"""
        button_width = 400
        button_height = 70
        spacing = 20
    ...
    def update(self, dt):
        """Actualiza la pantalla de configuración"""
        for button in self.buttons:
            button.update(dt)
    ...
    def draw(self):
        ...
        # Ayuda
        font_pequena = self.resource_manager.get_font('pequena')
        if font_pequena:
            help_surface = font_pequena.render("Presiona ESC para volver", True, COLOR_TEXTO_SUTIL_EN_FONDO)
